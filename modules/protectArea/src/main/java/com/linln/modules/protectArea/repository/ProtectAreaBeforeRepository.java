 ...
 * @date 2020/11/26
 */
public interface ProtectAreaBeforeRepository extends BaseRepository<ProtectAreaBefore, Long> {
    @Query(nativeQuery = true,value = "select distinct pa.level from pa_protect_area_before pa")
    List getLevel();

    @Query(nativeQuery = true,value = "" +
            "select ROUND(sum(pa.current_area),2) as currentAreaSum from pa_protect_area_before pa" +
            " where pa.level = :level"
    )
    List getAreaStatsByLevel(@Param("level")String level);

    @Query(nativeQuery = true,value = "" +
            "select ROUND(sum(pa.current_area),2) as currentAreaSum, pa.level from pa_protect_area_before pa" +
            " group by pa.level"
    )
    ...
    @Query(nativeQuery = true, value = "SELECT " +
            ...
            "t1.integration as integration, " +
            "t1.remarks, " +
            "t1.create_date as createDate, " +
            "t1.update_date as updateDate, " +
            "t1.status, " +
            "t1.location_count as locationCount " +
            "FROM pa_protect_area_before t1, pa_location_before t2 where " +
            "t1.id = t2.protect_area_before_id and " +
    ...
    @Query(nativeQuery = true,value = "" +
            "select count(*) as count from pa_protect_area_before pa"
    )
    String getCount();
    ...
    @Query(nativeQuery = true,value = "select distinct pa.category from pa_protect_area_before pa")
    List getCategory();
}